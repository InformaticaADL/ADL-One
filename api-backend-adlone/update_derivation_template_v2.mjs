...
async function check() {
    try {
        ...
        const q1 = `
            ...
            SET 
                cuerpo_template_html = @htmlContent,
                asunto_template = 'Notificación: Solicitud Derivada ({nombre_tipo})'
            WHERE codigo_evento = 'SOLICITUD_DERIVACION'
        `;
        await pool.request()
            .input('htmlContent', sql.NVarChar(sql.MAX), HTML_CONTENT)
            .query(q1);
        
        console.log('Template HTML actualizado a la versión oficial provista por el usuario.');
