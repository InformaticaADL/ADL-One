...
export const SolicitudesMaPage: React.FC<Props> = ({ onBack, viewOnly = false }) => {
    ...
    const canViewReportDetails = hasPermission('MA_A_REPORTES_DETALLE') || isSuperAdmin;
    const canRequestReportRevision = hasPermission('MA_A_REPORTES_REVISION') || isSuperAdmin;

    // Granular AI Permissions
    const canCreateRequest = hasPermission('AI_MA_CREAR_SOLICITUD') || isSuperAdmin;
    ...
    const getTipoLabel = (tipo: string) => {
        if (tipo === 'ALTA') return 'Activación';
    ...
    const getStatusLabel = (status: string) => {
        switch (status) {
            case 'EN_REVISION_TECNICA': return 'EN REVISIÓN ÁREA TÉCNICA';
            case 'PENDIENTE_TECNICA': return 'PENDIENTE ÁREA TÉCNICA';
            case 'PENDIENTE_CALIDAD': return 'PENDIENTE ÁREA CALIDAD';
            case 'EN_REVISION_CALIDAD': return 'EN REVISIÓN ÁREA CALIDAD';
            case 'RECHAZADO_TECNICA': return 'RECHAZADA ÁREA TÉCNICA';
            case 'RECHAZADO_CALIDAD': return 'RECHAZADA ÁREA CALIDAD';
            case 'RECHAZADO': return 'RECHAZADA ÁREA CALIDAD';
            case 'APROBADO_TECNICA': return 'APROBADO POR ÁREA TÉCNICA';
            case 'APROBADO_CALIDAD': return 'APROBADO POR ÁREA CALIDAD';
            case 'APROBADO': return 'APROBADO POR ÁREA CALIDAD';
            default: return status.replace(/_/g, ' ');
        }
    };

    const getSolicitanteNombre = (sol: any) => {
        const fallbacks = [
            sol?.nombre_solicitante,
            sol?.nombre_completo,
            sol?.usuario_nombre,
            sol?.usuario_creacion_nombre,
            sol?.usuario_solicita,
            sol?.datos_json?.responsable,
            sol?.datos_json?.solicitante,
            sol?.id_usuario_creacion,
            sol?.usuario_id
        ];
        
        for (const rawName of fallbacks) {
            if (rawName !== null && rawName !== undefined && String(rawName).trim() !== '') {
                const trimmed = String(rawName).trim();
                // Check if it's a numeric ID (like "394")
                if (/^\d+$/.test(trimmed)) {
                    // 0. Look up in global usuarios array (admin/tech)
                    const foundUser = usuariosDB.find((u: any) => String(u.id_usuario) === trimmed);
                    if (foundUser && (foundUser.nombre_real || foundUser.usuario)) {
                        return foundUser.nombre_real || foundUser.usuario;
                    }

                    // 1. Look up in the loaded muestreadores array (field users)
                    const found = muestreadores.find((m: any) => 
                        String(m.id_muestreador) === trimmed || 
                        String(m.id_usuario) === trimmed
                    );
                    if (found && (found.nombre_muestreador || found.nombre)) {
                        return found.nombre_muestreador || found.nombre;
                    }
                    
                    // 2. Check if it's the current logged in user
                    const u = user as any;
                    if (u && (String(u.id_usuario) === trimmed || String(u.id) === trimmed)) {
                        return u.nombre || u.usuario || trimmed;
                    }
                    
                    // If not found in mapping, we don't break, we let it continue to see if another string field has a real name
                } else {
                    // Try to resolve shortened/login names (like "pflores") into full names
                    const upperTrimmed = trimmed.toUpperCase();
                    
                    const foundUser = usuariosDB.find((u: any) => {
                        const isAlias = (u.nombre_real && String(u.nombre_real).trim().toUpperCase() === upperTrimmed) ||
                                        (u.usuario && String(u.usuario).trim().toUpperCase() === upperTrimmed);
                        const isFullName = (u.nombre_usuario && String(u.nombre_usuario).trim().toUpperCase() === upperTrimmed);
                        return isAlias || isFullName;
                    });

                    if (foundUser) {
                        // Return the actual human name (nombre_real), fallback to the alias if missing
                        const realName = foundUser.nombre_real || foundUser.usuario;
                        if (realName && String(realName).trim() !== '') {
                            return String(realName).trim();
                        }
                    }
                    
                    const foundM = muestreadores.find((m: any) => 
                        (m.nombre_muestreador && String(m.nombre_muestreador).toUpperCase() === upperTrimmed) ||
                        (m.nombre && String(m.nombre).toUpperCase() === upperTrimmed)
                    );
                    if (foundM && (foundM.nombre_muestreador || foundM.nombre)) {
                        return foundM.nombre_muestreador || foundM.nombre;
                    }

                    return trimmed; // It's a real name without a known expansion
                }
            }
        }
        
        // If all fallbacks failed to yield a pure name, return the first valid non-empty string as a last resort
        const firstValid = fallbacks.find(n => n !== null && n !== undefined && String(n).trim() !== '');
        return firstValid !== undefined ? String(firstValid).trim() : 'N/A';
    };

    const renderEquipmentInfo = (sol: any, equiposList?: any[]) => {
        const data = sol.datos_json;
        if (!data) return { code: 'N/A', name: 'Sin datos' };

        const list = equiposList || [];
        // Helper to find by ID
        const eqInfo = data.id_equipo && list.length ? list.find(e => String(e.id_equipo) === String(data.id_equipo)) : null;

        // Tipos con múltiples equipos
        if (sol.tipo_solicitud === 'BAJA') {
            if (data.equipos_baja?.length > 0) {
                const first = data.equipos_baja[0];
                const extra = data.equipos_baja.length - 1;
                
                let c = first.codigo || first.datos_originales?.codigo || 'N/A';
                let n = first.nombre || 'Sin nombre';
                
                if (c === 'N/A' && typeof n === 'string' && n.endsWith(')')) {
                    const match = n.match(/\(([^)]+)\)$/);
                    if (match) {
                        c = match[1];
                        n = n.replace(/\s*\([^)]+\)$/, '');
                ...
                return {
                    code: c,
                    name: `${n}${extra > 0 ? ` (+${extra})` : ''}`
                };
            }
            return {
                code: data.codigo || data.codigo_equipo || data['Código'] || data.codigo_interno || eqInfo?.codigo || 'N/A',
                name: data.nombre || data.nombre_equipo || data['Nombre'] || data.nombre_interno || eqInfo?.nombre || 'Sin nombre'
            };
        }

        if (sol.tipo_solicitud === 'ALTA') {
            if (data.equipos_alta?.length > 0) {
                ...
                // If code is still N/A, try to extract from name "Name (Code)"
                if (c === 'N/A' && typeof n === 'string' && n.endsWith(')')) {
                    const match = n.match(/\(([^)]+)\)$/);
                    if (match) {
                        c = match[1];
            ...
            // For single activations, we can also look at top level data.codigo if data.codigo_equipo is empty
            return {
                code: data.codigo || data.codigo_equipo || data['Código'] || data.codigo_interno || eqInfo?.codigo || 'N/A',
                name: data.nombre || data.nombre_equipo || data['Nombre'] || data.nombre_interno || eqInfo?.nombre || 'Sin nombre'
            };
        ...
        const code = data.codigo || data.equipo_codigo || data.codigo_equipo || eqInfo?.codigo || 'N/A';
        const name = data.nombre || data.equipo_nombre || data.nombre_equipo || eqInfo?.nombre || 'Sin nombre';
    ...
    const matchesFilters = (sol: any) => {
        ...
        if (filterStatus && filterStatus !== 'Todos') {
            if (getStatusLabel(sol.estado) !== filterStatus) return false;
        }

        return true;
    };

    const filteredIncoming = useMemo(() => incomingRequests.filter(matchesFilters), [incomingRequests, searchTerm, filterTipo, filterEquipo, filterStatus, equipos]);
    const filteredOutgoing = useMemo(() => outgoingRequests.filter(matchesFilters), [outgoingRequests, searchTerm, filterTipo, filterEquipo, filterStatus, equipos]);
    const filteredVouchers = useMemo(() => qualityVouchers.filter(matchesFilters), [qualityVouchers, searchTerm, filterTipo, filterEquipo, filterStatus, equipos]);
    ...
    }, [reviewedHistory, respondedHistory, searchTerm, filterTipo, filterEquipo, filterStatus, equipos]);
    ...
    useEffect(() => {
        ...
        setVoucherPage(0);
        setHistorialPage(0);
    }, [searchTerm, filterTipo, filterEquipo, filterStatus]);
    ...
    useEffect(() => {
        if (viewOnly && activeTab !== 'POR_VALIDAR') {
            setActiveTab('POR_VALIDAR');
        }
    }, [viewOnly, activeTab]);
    ...
    const loadHistory = async () => {
        try {
            ...
            // For MA (Technical): ONLY PENDIENTE_TECNICA
            ...
            if (isSuperAdmin) {
                ...
                if (isMAMan) incomingStatesArr.push('PENDIENTE_TECNICA');
                if (isGCMan) incomingStatesArr.push('PENDIENTE_CALIDAD');
            }

            const incoming = await adminService.getSolicitudes({
                estado: incomingStatesArr.join(',') || 'PENDIENTE_TECNICA'
            });

            // Filtering for specific management types if needed
            const managementTypes = [
                'ALTA', 'BAJA', 'TRASPASO', 'VIGENCIA_PROXIMA',
                'EQUIPO_PERDIDO', 'NUEVO_EQUIPO', 'CONSULTA_GENERAL',
                'EQUIPO_DESHABILITADO'
                // REPORTE_PROBLEMA and REVISION are excluded to only appear in Reports Tab
            ];
            setIncomingRequests(incoming.filter((s: any) => managementTypes.includes(s.tipo_solicitud)));

            // 2. En Revisión (Outgoing Tab): Work currently being processed by Technical Area
            if (isMAMan || isSuperAdmin) {
                const outgoing = await adminService.getSolicitudes({ estado: 'EN_REVISION_TECNICA' });
                setOutgoingRequests(outgoing);
            } else {
                setOutgoingRequests([]);
            }

            // 3. Technical History: Work finished by Technical Area (Derived or Rejected)
            // 4. Final History: Finalized by Quality
            let historyStates = ['APROBADO', 'RECHAZADO', 'RECHAZADO_TECNICA'];
            // If I am Technical, PENDIENTE_CALIDAD is part of MY history (I already did my part)
            if (isMAMan || isSuperAdmin) {
                historyStates.push('PENDIENTE_CALIDAD');
            }

            const historyRes = await adminService.getSolicitudes({
                estado: historyStates.join(',')
            });

            // Split into reviewed and responded for the combined state
            setReviewedHistory(historyRes.filter((s: any) => s.estado === 'RECHAZADO_TECNICA' || s.estado === 'PENDIENTE_CALIDAD'));
            setRespondedHistory(historyRes.filter((s: any) => s.estado === 'APROBADO' || s.estado === 'RECHAZADO'));

            // 5. Quality Vouchers: Evidence view
            const vouchers = await adminService.getSolicitudes({
                estado: 'PENDIENTE_TECNICA,EN_REVISION_TECNICA,PENDIENTE_CALIDAD,APROBADO,RECHAZADO,RECHAZADO_TECNICA'
            });
            const voucherTypes = ['EQUIPO_PERDIDO', 'REPORTE_PROBLEMA', 'REVISION'];
            setQualityVouchers(vouchers.filter((v: any) => voucherTypes.includes(v.tipo_solicitud)));
    ...
    const loadInitialData = async () => {
        ...
        try {
            ...
            try {
                const usrRes = await adminService.getUsuarios();
                console.log('DEBUG USRRES:', usrRes);
                setUsuariosDB(usrRes.data || usrRes || []);
            } catch (err) {
            ...
            setQualityVouchers(prev => prev.map(v =>
                v.id_solicitud === selectedRequest.id_solicitud
                    ? {
                        ...
                        feedback_aprobacion: reviewFeedback,
    ...
    return (
        <div className="admin-container" style={{ paddingTop: '0.25rem', height: '100dvh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ padding: '0.25rem 1rem 0', background: 'transparent', zIndex: 10 }}>
                        ...
                        {viewMode === 'LIST' && !viewOnly ? (
                            <button
                                ...
                                style={{
                                    ...
                                    opacity: !canCreateRequest ? 0.5 : 1,
                                    cursor: !canCreateRequest ? 'not-allowed' : 'pointer'
                                }}
                            >
                ...
                {viewMode === 'LIST' && (
                    <>
                        {/* Tabs Control */}
                        {!viewOnly && (
                            <div className="tabs-container" style={{ display: 'flex', justifyContent: 'center', borderBottom: '1px solid #e2e8f0', marginBottom: '1rem', overflowX: 'auto', WebkitOverflowScrolling: 'touch', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                                <button
                                ...
                                <button
                                    onClick={() => canCreateRequest && setActiveTab('REVISION')}
                                    disabled={!canCreateRequest}
                                    title={!canCreateRequest ? 'No tienes permisos para ver las solicitudes en revisión' : ''}
                                    style={{
                                        padding: '0.6rem 1.2rem',
                                        background: 'none',
                                        border: 'none',
                        ...
                        {/* Tabs Control for Read-Only (Reportes) */}
                        {viewOnly && (
                            <div className="tabs-container" style={{ display: 'flex', justifyContent: 'center', borderBottom: '1px solid #e2e8f0', marginBottom: '1rem', overflowX: 'auto', WebkitOverflowScrolling: 'touch', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                                <button
                                    ...
                                    style={{
                                        ...
                                        background: 'none',
                                        border: 'none',
                                        borderBottom: activeTab === 'POR_VALIDAR' ? '2px solid #2563eb' : '2px solid transparent',
                                        color: activeTab === 'POR_VALIDAR' ? '#2563eb' : '#64748b',
                                        fontWeight: activeTab === 'POR_VALIDAR' ? 700 : 500,
                                        cursor: 'pointer',
                                        transition: 'all 0.2s',
                                        display: 'flex',
                                        alignItems: 'center',
                        ...
                        <div className={`filter-card ${showMobileFilters ? 'mobile-expanded' : ''}`} style={{
                            ...
                            boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05)',
                            padding: '0.5rem 1rem',
                            maxWidth: '1500px',
                                    ...
                                    <div className="filter-collapsible-area">
                                        <div className="filter-group-content">
                                            <CustomSelect
                                                ...
                                                options={viewOnly ?
                                                    ['Todos', 'Baja por Pérdida', 'Reporte de Problema', 'Revisión'] :
                                                    ['Todos', 'Baja por Pérdida', 'Reporte de Problema', 'Equipo Deshabilitado', 'Revisión', 'Traspaso', 'Activación', 'Vigencia Próxima']
                                                }
                                            ...
                                            <CustomSelect
                                                label="Estado"
                                                value={filterStatus || 'Estado'}
                                                options={['Todos', 'APROBADO', 'PENDIENTE']}
                                                onChange={(v) => { setFilterStatus(v === 'Todos' ? '' : v); setIncomingPage(0); setOutgoingPage(0); setVoucherPage(0); setHistorialPage(0); }}
                                                width="120px"
                                            />
                                            {hasActiveFilters && (
            ...
            <div className="admin-content-scroll" style={{ flex: 1, overflowY: 'auto', padding: '0 1rem 1rem', background: 'transparent', position: 'relative' }}>
                ...
                {loading ? (
                    <div style={{
                        ...
                        <p style={{ marginTop: '1rem', color: '#64748b', fontWeight: 500 }}>Cargando información...</p>
                        <style>{`
                            @keyframes spin {
                    ...
                    <>
                        {viewMode === 'LIST' && (
                            <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', minWidth: 0, width: '95%', maxWidth: '1500px', margin: '0 auto' }}>
                                                                ...
                                                                return (
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <td style={{ textAlign: 'left' }}>
                                                                            {(() => {
                                                                                const info = renderEquipmentInfo(sol, equipos);
                                                                                return (
                                                                                    <>
                                                                                        <div style={{ fontWeight: 600, color: '#1e293b', fontSize: '0.85rem' }}>
                                                                                            {info.code}
                                                        ...
                                                        <tbody>
                                                            {filteredOutgoing.slice(outgoingPage * ITEMS_PER_PAGE, (outgoingPage + 1) * ITEMS_PER_PAGE).map(sol => (
                                                                <tr key={sol.id_solicitud}
                                                                    onClick={() => {
                                                                        setSelectedRequest(sol);
                                                                        setShowReviewModal(true); // Open Review Modal instead of Detail logic
                                                                        setReviewAction(null);
                                                                        setReviewFeedback('');
                                                                    }}
                                                                    ...
                                                                    <td style={{ textAlign: 'center' }}>
                                                                        <div style={{ display: 'flex', justifyContent: 'center', width: '100%' }}>
                                                                            <button
                                                                                ...
                                                                                style={{ padding: '0.3rem 0.6rem', fontSize: '0.7rem', background: '#10b981' }}
                                                                                onClick={(e) => {
                                                                                    e.stopPropagation();
                                                                                    setSelectedRequest(sol);
                                                                                    setShowReviewModal(true); // Ensure processing button opens Review Modal
                                                                                    setReviewAction(null);
                                                                                    setReviewFeedback('');
                                                                                }}
                                                                            >
                                                                                {(isGCMan || isSuperAdmin) && sol.estado === 'PENDIENTE_CALIDAD' ? 'Procesar' : 'Ver Detalles'}
                                                                            </button>
                                                                        </div>
                                                                    </td>
                                                                </tr>
                                                        ...
                                                        <tbody>
                                                            {filteredVouchers.slice(voucherPage * ITEMS_PER_PAGE, (voucherPage + 1) * ITEMS_PER_PAGE).map(voucher => (
                                                                ...
                                                                >
                                                                    ...
                                                                    <td style={{ textAlign: 'left' }}>
                                                                        {(() => {
                                                                            const info = renderEquipmentInfo(voucher, equipos);
                                                                            return (
                                                                                <>
                                                                                    <div style={{ fontWeight: 600, color: '#1e293b', fontSize: '0.85rem' }}>
                                                                                        {info.code}
                                                            ...
                                                            <tbody>
                                                                {pagedHistory.map(sol => (
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <td style={{ padding: '1rem', color: '#64748b', fontSize: '0.85rem', verticalAlign: 'middle', textAlign: 'center' }}>
                                                                            {getSolicitanteNombre(sol)}
                                                                        </td>
                                                                        <td style={{ textAlign: 'center', fontSize: '0.8rem', color: '#475569' }}>
                                                                            {new Date(sol.fecha_solicitud).toLocaleDateString()}
                                                                        ...
                                                                        <td style={{ textAlign: 'center' }}>
                                                                            <div style={{ display: 'flex', justifyContent: 'center', width: '100%' }}>
                                                                                <button
                                                                                    className="btn-primary"
                                                                                    style={{ padding: '0.3rem 0.6rem', fontSize: '0.65rem', background: '#64748b' }}
                        ...
                        {viewMode === 'FORM' && (
                            <div className="form-card" style={{ maxWidth: '1500px', width: '95%', margin: '0 auto', background: 'white', padding: '2rem', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)' }}>
                                        ...
                                        <div className="modal-body" style={{ padding: '0 1.5rem 1.5rem 1.5rem', overflowY: 'auto', flex: 1 }}>
                                            ...
                                            <div style={{
                                                ...
                                                <div>
                                                    ...
                                                    <div style={{ fontWeight: 600, color: '#334155', fontSize: '0.95rem' }}>{getSolicitanteNombre(selectedRequest)}</div>
                                                </div>
                                                <div>
                                            ...
                                            <div style={{ marginTop: '1.5rem' }}>
                                                ...
                                                <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '1rem' }}>
                                                    ...
                                                    <div style={{ background: '#F8FAFC', padding: '1rem', borderRadius: '6px', color: '#334155', fontSize: '0.9rem', lineHeight: '1.5' }}>
                                                        ...
                                                        {selectedRequest.tipo_solicitud === 'REPORTE_PROBLEMA' && (
                                                            <div style={{ display: 'grid', gap: '1.25rem' }}>
                                                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
                                                                    <div>
                                                                        <strong style={{ display: 'block', fontSize: '0.7rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.025em', marginBottom: '0.25rem' }}>Tipo Problema</strong>
                                                                        <span style={{ color: '#0f172a', fontWeight: 600, fontSize: '0.95rem' }}>{selectedRequest.datos_json?.tipo_problema?.replace(/_/g, ' ')}</span>
                                                                    </div>
                                                                    <div>
                                                                        <strong style={{ display: 'block', fontSize: '0.7rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.025em', marginBottom: '0.25rem' }}>Severidad</strong>
                                                                        <span style={{
                                                                            ...
                                                                            background: selectedRequest.datos_json?.severidad === 'CRITICA' ? '#fef2f2' : 'transparent',
                                                                            padding: selectedRequest.datos_json?.severidad === 'CRITICA' ? '2px 8px' : '0',
                                                                            borderRadius: '4px'
                                                                        }}>
                                                                            {selectedRequest.datos_json?.severidad === 'CRITICA' && <span style={{ fontSize: '1.1rem' }}>?</span>}
                                                                            {selectedRequest.datos_json?.severidad}
                                                                        </span>
                                                                    </div>
                                                                    <div>
                                                                        <strong style={{ display: 'block', fontSize: '0.7rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.025em', marginBottom: '0.25rem' }}>Frecuencia</strong>
                                                                        <span style={{ color: '#334155', fontWeight: 500 }}>{selectedRequest.datos_json?.frecuencia}</span>
                                                                    </div>
                                                                    <div>
                                                                        <strong style={{ display: 'block', fontSize: '0.7rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.025em', marginBottom: '0.25rem' }}>Ubicación</strong>
                                                                        <span style={{ color: '#334155', fontWeight: 500 }}>{selectedRequest.datos_json?.ubicacion || equipos.find(e => String(e.id_equipo) === String(selectedRequest.datos_json?.id_equipo))?.ubicacion || '-'}</span>
                                                                    </div>
                                                                    <div>
                                                                        <strong style={{ display: 'block', fontSize: '0.7rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.025em', marginBottom: '0.25rem' }}>Afecta Mediciones</strong>
                                                                        <span style={{
                                                                            color: selectedRequest.datos_json?.afecta_mediciones ? '#dc2626' : '#16a34a',
                                                                            fontWeight: 600
                                                                        }}>
                                                                            {selectedRequest.datos_json?.afecta_mediciones ? 'SÍ' : 'NO'}
                                                                        </span>
                                                                    </div>
                                                                </div>

                                                                {selectedRequest.datos_json?.sintomas && (
                                                                    <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '1rem' }}>
                                                                        <strong style={{ display: 'block', fontSize: '0.7rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.025em', marginBottom: '0.5rem' }}>Síntomas Observados</strong>
                                                                        <div style={{ background: '#fdf2f280', padding: '0.75rem 1rem', borderRadius: '8px', border: '1px solid #fee2e2', color: '#7f1d1d', fontSize: '0.9rem', fontStyle: 'italic', lineHeight: '1.5' }}>
                                                                            {selectedRequest.datos_json?.sintomas}
                                                                        </div>
                                                                    </div>
                                                                )}
                                                                ...
                                                                <div>
                                                                    <strong style={{ display: 'block', fontSize: '0.7rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.025em', marginBottom: '0.5rem' }}>Descripción Detallada</strong>
                                                                    <div style={{ background: 'white', padding: '1rem', borderRadius: '8px', border: '1px solid #e2e8f0', color: '#1e293b', fontSize: '0.95rem', lineHeight: '1.6', boxShadow: 'inset 0 1px 2px rgba(0,0,0,0.02)' }}>
                                                        ...
                                                        {selectedRequest.tipo_solicitud === 'REVISION' && (
                                                            <div style={{ display: 'grid', gap: '1.25rem' }}>
                                                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem', paddingBottom: '1rem', borderBottom: '1px solid #f1f5f9' }}>
                                                                    <div>
                                                                        <strong style={{ display: 'block', fontSize: '0.7rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.025em', marginBottom: '0.25rem' }}>Motivo Revisión</strong>
                                                                        <span style={{ color: '#0f172a', fontWeight: 600, fontSize: '0.95rem' }}>{selectedRequest.datos_json?.motivo_revision}</span>
                                                                    </div>
                                                                    <div>
                                                                        <strong style={{ display: 'block', fontSize: '0.7rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.025em', marginBottom: '0.25rem' }}>Urgencia</strong>
                                                                        <span style={{
                                                                            color: selectedRequest.datos_json?.urgencia === 'ALTA' ? '#dc2626' :
                                                                                selectedRequest.datos_json?.urgencia === 'MEDIA' ? '#d97706' : '#16a34a',
                                                                            fontWeight: 700,
                                                                            fontSize: '0.95rem',
                                                                            background: selectedRequest.datos_json?.urgencia === 'ALTA' ? '#fef2f2' :
                                                                                selectedRequest.datos_json?.urgencia === 'MEDIA' ? '#fffbeb' : '#f0fdf4',
                                                                            padding: '2px 10px',
                                                                            borderRadius: '12px'
                                                                        }}>
                                                                ...
                                                                <div>
                                                                    <strong style={{ display: 'block', fontSize: '0.7rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.025em', marginBottom: '0.5rem' }}>Instrucciones / Detalle</strong>
                                                                    <div style={{ background: 'white', padding: '1rem', borderRadius: '8px', border: '1px solid #e2e8f0', color: '#1e293b', fontSize: '0.95rem', lineHeight: '1.6' }}>
                                                                        {selectedRequest.datos_json?.descripcion}
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        ...
                                                        {selectedRequest.tipo_solicitud === 'EQUIPO_PERDIDO' && (() => {
                                                            ...
                                                            const nombre = eqData?.nombre || 'N/A';
                                                            const codigo = eqData?.codigo || 'N/A';
                                                            const tipo = eqData?.tipo || 'N/A';
                                                            const ubicacionStr = eqData?.ubicacion || 'N/A';
                                                            const responsable = eqData?.nombre_asignado || 'N/A';

                                                            const tipoPerdida = selectedRequest.datos_json?.tipo_perdida || 'N/A';
                                                            const fechaIncidente = selectedRequest.datos_json?.fecha_incidente ? new Date(selectedRequest.datos_json.fecha_incidente).toLocaleDateString() : 'N/A';
                                                                ...
                                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                                                                    ...
                                                                    <div style={{ display: 'flex', gap: '1rem', background: '#fef2f2', padding: '1rem', borderRadius: '8px', borderLeft: '4px solid #ef4444' }}>
                                                                        <div style={{ flex: 1 }}>
                                                                            ...
                                                                            <span style={{ fontSize: '1.1rem', color: '#b91c1c', fontWeight: 800 }}>{tipoPerdida}</span>
                                                                        </div>
                                                                        <div style={{ flex: 1 }}>
                                                                            ...
                                                                            <span style={{ fontSize: '1.1rem', color: '#b91c1c', fontWeight: 800 }}>{fechaIncidente}</span>
                                                                        </div>
                                                                    </div>
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1.25rem' }}>
                                                                            <div>
                                                                                ...
                                                                                <span style={{ color: '#1e293b', fontWeight: 500 }}>{selectedRequest.datos_json?.ubicacion_ultima || ubicacionStr}</span>
                                                                            </div>
                                                                            <div>
                                                                                ...
                                                                                <p style={{ margin: 0, color: '#334155', background: '#f1f5f9', padding: '0.75rem', borderRadius: '6px', fontSize: '0.9rem', lineHeight: '1.5' }}>
                                                                                    {selectedRequest.datos_json?.circunstancias || 'No se detallaron circunstancias.'}
                                                                                </p>
                                                                            </div>
                                                                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                                                                <div>
                                                                                    ...
                                                                                    <p style={{ margin: 0, color: '#475569', fontSize: '0.9rem' }}>{selectedRequest.datos_json?.acciones_tomadas || 'Ninguna'}</p>
                                                                                </div>
                                                                                <div>
                                                                                    <strong style={{ display: 'block', fontSize: '0.7rem', color: '#64748b', textTransform: 'uppercase', marginBottom: '0.25rem' }}>Testigos</strong>
                                                                                    <p style={{ margin: 0, color: '#475569', fontSize: '0.9rem' }}>{selectedRequest.datos_json?.testigos || 'Sin testigos'}</p>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                ...
                                                {(!viewOnly && activeTab !== 'HISTORIAL' && (
                                                    ((selectedRequest.estado === 'PENDIENTE_TECNICA' || selectedRequest.estado === 'EN_REVISION_TECNICA') && (isMAMan || isSuperAdmin)) ||
                                                    (selectedRequest.estado === 'PENDIENTE_CALIDAD' && (isGCMan || isSuperAdmin || isMAMan))
                                                )) && (
                                                        <div style={{ marginTop: '1.5rem', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '1.25rem' }}>
                                                            ...
                                                            {(() => {
                                                                const isCalidadThreeOption = ['REPORTE_PROBLEMA', 'EQUIPO_DESHABILITADO', 'REVISION', 'CONSULTA_GENERAL'].includes(selectedRequest.tipo_solicitud);
                                                                const showThreeOptions = selectedRequest.estado === 'EN_REVISION_TECNICA' || (selectedRequest.estado === 'PENDIENTE_CALIDAD' && isCalidadThreeOption);

                                                                return (
                                                                    <div style={{ display: 'grid', gridTemplateColumns: showThreeOptions ? '1fr 1fr 1fr' : '1fr 1fr', gap: '0.75rem', marginBottom: '1.25rem' }}>
                                                                        ...
                                                                        >
                                                                            ...
                                                                            <span style={{ fontWeight: 700, fontSize: '0.8rem', color: reviewAction === 'DERIVADO' ? '#15803d' : '#64748b' }}>
                                                                                {selectedRequest.estado === 'PENDIENTE_TECNICA' ? 'Aceptar' : 'Derivar'}
                                                                            </span>
                                                                        </div>

                                                                        {/* Approve & Conclude Button / Tile */}
                                                                        {(selectedRequest.estado === 'EN_REVISION_TECNICA' || (selectedRequest.estado === 'PENDIENTE_CALIDAD' && isCalidadThreeOption)) && (
                                                                            <div
                                                                                onClick={() => {
                                                                            ...
                                                                            >
                                                                                ...
                                                                                <span style={{ fontWeight: 700, fontSize: '0.8rem', color: reviewAction === 'CONCLUIDO' ? '#1d4ed8' : '#64748b' }}>
                                                                                    {selectedRequest.estado === 'EN_REVISION_TECNICA' ? 'Concluir' : (selectedRequest.tipo_solicitud === 'EQUIPO_DESHABILITADO' ? 'Solucionado' : 'Aprobar sin derivar')}
                                                                                </span>
                                                                            </div>
                                                                        )}
                                                                        ...
                                                                        >
                                                                            ...
                                                                            <span style={{ fontWeight: 700, fontSize: '0.8rem', color: reviewAction === 'RECHAZADO' ? '#b91c1c' : '#64748b' }}>
                                                                                {selectedRequest.tipo_solicitud === 'EQUIPO_DESHABILITADO' ? 'Rechazar' : 'Rechazar'}
                                                                            </span>
                                                                        </div>
                                                                    </div>
                                                                );
                                                            ...
                                                            {reviewAction && (
                                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                                                    {(selectedRequest.tipo_solicitud === 'VIGENCIA_PROXIMA' || selectedRequest.tipo_solicitud === 'ALTA') && reviewAction === 'DERIVADO' && selectedRequest.estado === 'EN_REVISION_TECNICA' && (
                                                                        <div>
                                                                            <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, color: '#475569', marginBottom: '0.5rem' }}>
                                                                                {selectedRequest.tipo_solicitud === 'ALTA' ? 'Fecha de Activación/Vigencia' : 'Nueva Fecha de Vigencia'} <span style={{ color: '#ef4444' }}>*</span>
                                                                            </label>
                                                                            <input
                                                                                type="date"
                                                                                className="form-input"
                                                                                min={new Date().toISOString().split('T')[0]}
                                                                                value={editedData?.nueva_vigencia_solicitada || ''}
                                                ...
                                                {(['PENDIENTE_CALIDAD', 'EN_REVISION_TECNICA'].includes(selectedRequest.estado?.trim()) &&
                                            ...
                                            <div className="modal-footer" style={{ padding: '1rem 1.5rem', borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', background: 'white' }}>
                                                ...
                                                >
                                                    {isReplying ? 'Cancelar' : (viewOnly || activeTab === 'HISTORIAL') ? 'Cerrar' : 'Cancelar'}
                                                </button>

                                                {/* Voucher Review Flow (Solicitar Revisión) */}
                                                {((viewOnly || activeTab === 'POR_VALIDAR' || activeTab === 'REVISION' || activeTab === 'PENDIENTES') &&
                                                    ['REPORTE_PROBLEMA', 'REVISION'].includes(selectedRequest.tipo_solicitud) &&
                                                    ['PENDIENTE_CALIDAD', 'EN_REVISION_TECNICA'].includes(selectedRequest.estado?.trim()) &&
                                                    activeTab !== 'HISTORIAL' && (viewOnly ? canRequestReportRevision : true)) && (
                                                        <>
                                                            {!isReplying ? (
                                                                <button
                                                                    onClick={() => {
                                                ...
                                                {(!viewOnly && activeTab !== 'HISTORIAL') && (
                                                    <>
                                                        {/* Reply Flow Buttons (Quality/Lead ONLY) */}
                                                        {(selectedRequest.estado === 'PENDIENTE_CALIDAD' &&
                                                            (isGCMan || isMAMan || isSuperAdmin) &&
                                                            ['REPORTE_PROBLEMA', 'REVISION'].includes(selectedRequest.tipo_solicitud)) ? (
                                                            <>
                                                                {!isReplying ? (
                                                                    <button
                                                            ...
                                                            /* Standard Review Button (Enables when action is selected in EN_REVISION_TECNICA or PENDIENTE_TECNICA) */
                                                            ((['PENDIENTE_TECNICA', 'EN_REVISION_TECNICA'].includes(selectedRequest.estado) && (isMAMan || isSuperAdmin)) ||
                                                                (selectedRequest.estado === 'PENDIENTE_CALIDAD' && (isGCMan || isMAMan || isSuperAdmin))) && (
                                                                <button
                                                                    onClick={submitTechnicalReview}
                                                                    disabled={
                                                                        !reviewAction ||
                                                                        !reviewFeedback.trim() ||
                                                                        (reviewAction === 'DERIVADO' && selectedRequest.estado === 'EN_REVISION_TECNICA' && (selectedRequest.tipo_solicitud === 'VIGENCIA_PROXIMA' || selectedRequest.tipo_solicitud === 'ALTA') && !editedData?.nueva_vigencia_solicitada) ||
                                                                        loading
                                                                    }
                                                                    style={{
                                                                        padding: '0.6rem 1.25rem',
                                                                        borderRadius: '6px',
                                                                        border: 'none',
                                                                        background: (!reviewAction || !reviewFeedback.trim()) ? '#94a3b8' : '#64748b',
                                                                        color: 'white',
                                                                        fontWeight: 600,
                                                                        fontSize: '0.9rem',
                                                                        cursor: (!reviewAction || !reviewFeedback.trim() || loading) ? 'not-allowed' : 'pointer',
                                                                    }}
                                                                >
                                                                    {loading ? 'Procesando...' : (
                                                                        reviewAction === 'DERIVADO' ? 'Confirmar Derivación' :
                                                                            reviewAction === 'CONCLUIDO' ? 'Confirmar Solución' :
                                                                                reviewAction === 'RECHAZADO' ? 'Confirmar Rechazo' :
                                                                                    'Confirmar Revisión'
                                                                    )}
                                                                </button>
                                                            )
                                ...
                                <div className="modal-overlay" style={{ zIndex: 10001, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0, 0, 0, 0.5)', backdropFilter: 'blur(4px)' }}>
                                    <div className="modal-content" style={{
                                        ...
                                        <div className="modal-body" style={{ padding: '0 1.5rem 1.5rem 1.5rem', overflowY: 'auto', flex: 1 }}>
                                            ...
                                            <div style={{ marginTop: '1.5rem' }}>
                                                ...
                                                <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '1rem' }}>
                                                    ...
                                                    {(selectedRequest.feedback_admin || selectedRequest.feedback_aprobacion || (selectedRequest.estado === 'APROBADO' && selectedRequest.usuario_aprueba_nombre)) && (
                                                        <div style={{ marginTop: '0.75rem', padding: '1rem', background: '#F8FAFC', borderRadius: '6px', border: '1px solid #e2e8f0' }}>
                                                            ...
                                                            <p style={{ margin: '0 0 0.5rem 0', color: '#1e293b', fontSize: '0.9rem', fontWeight: 500 }}>
                                                                {selectedRequest.feedback_aprobacion || selectedRequest.feedback_admin || (selectedRequest.estado === 'APROBADO' ? 'Solicitud aprobada correctamente.' : 'Sin comentarios adicionales.')}
                                                            </p>
                                                    ...
                                                    {/* Approve & Conclude Button / Tile (Only for Group B: Technical Types) */}
                                                    {selectedRequest.estado === 'EN_REVISION_TECNICA' &&
                                                        ['REPORTE_PROBLEMA', 'REVISION', 'CONSULTA_GENERAL'].includes(selectedRequest.tipo_solicitud) &&
                                                        activeTab !== 'HISTORIAL' && (
                                                            <div
                                                                onClick={() => {
                                                                    setReviewAction('CONCLUIDO');
                                            ...
                                            {(selectedRequest.tipo_solicitud !== 'CONSULTA_GENERAL' && selectedRequest.estado === 'EN_REVISION_TECNICA' && !viewOnly && activeTab !== 'HISTORIAL') && (
                                                <div style={{ marginTop: '1.5rem' }}>
                                                        ...
                                                        <>
                                                            ...
                                                            <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '1rem' }}>
                                                                ...
                                                                <textarea
                                                                    ...
                                                                    onChange={(e) => setReviewFeedback(e.target.value)}
                                                                    placeholder={
                                                                        ((viewOnly || activeTab === 'POR_VALIDAR' || activeTab === 'REVISION' || activeTab === 'PENDIENTES') && ['REPORTE_PROBLEMA', 'REVISION'].includes(selectedRequest.tipo_solicitud))
                                                                            ? "Ej: 'Traer para revisión' u otras instrucciones..."
                                                                            : selectedRequest.tipo_solicitud === 'EQUIPO_PERDIDO'
                                                                                ? "Ingrese observaciones sobre la pérdida u otras instrucciones..."
                                                                                : selectedRequest.tipo_solicitud === 'VIGENCIA_PROXIMA'
                                        ...
                                        <div className="modal-footer" style={{
                                            ...
                                            {(selectedRequest.tipo_solicitud === 'CONSULTA_GENERAL' && selectedRequest.estado === 'EN_REVISION_TECNICA' && canAcceptRequest) ? (
                                                <>
                                                    <button
                                                ...
                                                <>
                                                    ...
                                                    >
                                                        {isReplying ? 'Cancelar' : 'Cerrar'}
                                                    </button>

                                                    {/* Voucher Review Flow (Solicitar Revisión) - NEW / CONSOLIDATED */}
                                                    {((viewOnly || activeTab === 'POR_VALIDAR' || activeTab === 'REVISION' || activeTab === 'PENDIENTES') &&
                                                        ['REPORTE_PROBLEMA', 'REVISION'].includes(selectedRequest.tipo_solicitud) &&
                                                        ['PENDIENTE_CALIDAD', 'EN_REVISION_TECNICA'].includes(selectedRequest.estado?.trim()) &&
                                                        activeTab !== 'HISTORIAL') && (
                                                    ...
                                                    {/* Special Footer for EQUIPO_DESHABILITADO Review (DEPRECATED - Moved to standard button below) */}
                                                    {/* Keeping empty conditional to avoid breaking structure if needed later, but standard button handles it now */}
                                                    {false && (
                                                        <button
                                                            onClick={() => {
                                                                if (!reviewFeedback.trim()) {
                                                            ...
                                                            disabled={loading || !reviewFeedback.trim() || !canAcceptRequest}
                                                            title={!canAcceptRequest ? 'No tienes permiso para concluir solicitudes' : ''}
                                                            style={{
                                                                padding: '0.6rem 1.25rem',
                                                                borderRadius: '6px',
                                                                border: 'none',
                                                                ...
                                                                fontWeight: 700,
                                                                fontSize: '0.9rem',
                                                                opacity: !canAcceptRequest ? 0.5 : 1,
                                                                cursor: (loading || !reviewFeedback.trim() || !canAcceptRequest) ? 'not-allowed' : 'pointer',
                                                                display: 'flex',
                                                                alignItems: 'center',
                                                                gap: '0.5rem'
                                                            }}
                                                    ...
                                                    {/* Derivation Action Button (DEPRECATED - Moved to standard button below) */}
                                                    {false && (
                                                        <button
                                                            onClick={async () => {
                                                                setLoading(true);
                                                            ...
                                                            disabled={loading || !canDeriveRequest ||
                                                            ...
                                                            title={!canDeriveRequest ? 'No tienes permiso para derivar solicitudes' : ''}
                                                            style={{
                                                                padding: '0.6rem 1.25rem',
                                                                borderRadius: '6px',
                                                                border: 'none',
                                                                ...
                                                                fontWeight: 700,
                                                                fontSize: '0.9rem',
                                                                opacity: !canDeriveRequest ? 0.5 : 1,
                                                                cursor: (loading || !canDeriveRequest ||
                                                                    (selectedRequest.tipo_solicitud === 'VIGENCIA_PROXIMA' && (!editedData?.nueva_vigencia_solicitada || !reviewFeedback.trim())) ||
                                                                    (selectedRequest.tipo_solicitud === 'EQUIPO_DESHABILITADO' && (!editedData?.nueva_vigencia_solicitada || !reviewFeedback.trim())) ||
                                                                    (selectedRequest.tipo_solicitud === 'EQUIPO_PERDIDO' && !reviewFeedback.trim()) ||
                                                    ...
                                                    {/* Reject Action Button (DEPRECATED - Moved to standard button below) */}
                                                    {false && (
                                                        <button
                                                            onClick={async () => {
                                                                setLoading(true);
                                                            ...
                                                            disabled={loading || !reviewFeedback.trim() || !canRejectRequest}
                                                            title={!canRejectRequest ? 'No tienes permiso para rechazar solicitudes' : ''}
                                                            style={{
                                                                padding: '0.6rem 1.25rem',
                                                                borderRadius: '6px',
                                                                border: 'none',
                                                                ...
                                                                fontWeight: 700,
                                                                fontSize: '0.9rem',
                                                                opacity: !canRejectRequest ? 0.5 : 1,
                                                                cursor: (loading || !reviewFeedback.trim() || !canRejectRequest) ? 'not-allowed' : 'pointer',
                                                                display: 'flex',
                                                                alignItems: 'center',
                                                                gap: '0.5rem'
                                                            }}
                                                    ...
                                                    {/* Standard Technical Review - Centralized Confirmation Button */}
                                                    {(['PENDIENTE_TECNICA', 'EN_REVISION_TECNICA'].includes(selectedRequest.estado) && (isMAMan || isSuperAdmin)) && (
                                                        <button
                                                            onClick={submitTechnicalReview}
                                                            disabled={
                                                                !reviewAction || !canAcceptRequest ||
                                                                !reviewFeedback.trim() ||
                                                                (reviewAction === 'DERIVADO' && selectedRequest.estado === 'EN_REVISION_TECNICA' && (selectedRequest.tipo_solicitud === 'VIGENCIA_PROXIMA' || selectedRequest.tipo_solicitud === 'ALTA') && !editedData?.nueva_vigencia_solicitada) ||
                                                                loading
                                                            }
                                                            title={!canAcceptRequest ? 'No tienes permiso para confirmar revisión' : !reviewFeedback.trim() ? 'El comentario es obligatorio' : ''}
                                                            style={{
                                                                padding: '0.6rem 1.25rem',
                                                                borderRadius: '6px',
                                                                border: 'none',
                                                                background: (!reviewAction || !reviewFeedback.trim() || !canAcceptRequest) ? '#94a3b8' :
                                                                    (reviewAction === 'RECHAZADO' ? '#ef4444' :
                                                                        reviewAction === 'DERIVADO' ? '#16a34a' : '#3b82f6'),
                                                                color: 'white',
                                                                fontWeight: 700,
                                                                fontSize: '0.9rem',
                                                                opacity: !canAcceptRequest ? 0.5 : 1,
                                                                cursor: (!reviewAction || !reviewFeedback.trim() || loading || !canAcceptRequest) ? 'not-allowed' : 'pointer',
                                                            }}
                                                        >
                                                            {loading ? 'Procesando...' : (
                                                                reviewAction === 'DERIVADO' ? 'Confirmar Derivación' :
