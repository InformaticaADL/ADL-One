...
>(({ className, ...props }, ref) => (
  <CommandPrimitive
    ref={ref}
    className={cn(
      'shadcn-scope flex h-full w-full flex-col overflow-hidden rounded-xl bg-card text-card-foreground',
      className
    )}
    {...props}
  />
...
// Título/descripción son sr-only por defecto (accesibilidad de Dialog) — el
// diálogo de comando no necesita un header visible, el input ya cumple ese rol.
function CommandDialog({ title = 'Buscar', description = 'Busca una página o sección', children, ...props }: CommandDialogProps) {
  return (
    <Dialog {...props}>
...
>(({ className, ...props }, ref) => (
  <CommandPrimitive.Item
    ref={ref}
    className={cn(
      'relative flex cursor-pointer select-none items-center gap-2 rounded-lg px-2 py-2 text-sm text-foreground outline-none data-[disabled=true]:pointer-events-none data-[selected=true]:bg-primary/10 data-[selected=true]:text-primary data-[disabled=true]:opacity-50',
      className
    )}
    {...props}
  />
