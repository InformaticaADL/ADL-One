...
dashboard_end = """    const renderEnvironmentDashboard = () => (
    ...
    const renderContent = () => {
        ...
        if (activeSubmodule === 'admin-equipos-gestion') return <EquiposPage onBack={() => setActiveSubmodule((activeModule as string) === 'gestion_calidad' ? 'gestion_calidad' : 'medio_ambiente')} />;
        ...
        if (!activeSubmodule) {
            // CASO A: Se presionó el logo de ADL One (!activeModule) -> Mostrar Dashboards Premium según rol
            if (!activeModule) {
            ...
            // El usuario prefiere mantener la vista original/genérica aquí
            return renderGenericDashboard();
        }
...
with codecs.open('src/pages/DashboardPage.tsx', 'r', 'utf-8') as f:
    text = f.read()

start_idx = text.find('    const renderEnvironmentDashboard = () => (')
if start_idx != -1:
    new_text = text[:start_idx] + dashboard_end
    with codecs.open('src/pages/DashboardPage.tsx', 'w', 'utf-8') as f:
        f.write(new_text)
    print("DashboardPage.tsx bottom half rewritten successfully.")
else:
